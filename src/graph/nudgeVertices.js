...
export const nudgeVerticesWithFaceOrders = ({ vertices_coords, faces_vertices, faceOrders }) => {
	const faces_normal = ear.graph.makeFacesNormal({ vertices_coords, faces_vertices });
	const sets_faces = ear.graph
		.getDisjointedVertices({ edges_vertices: faceOrders.map(ord => [ord[0], ord[1]]) });
